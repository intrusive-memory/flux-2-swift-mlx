...
struct TextToImage: AsyncParsableCommand {
  ...
  @Option(
    name: .long,
    help:
      "Model variant: dev (32B), klein-4b (4B, Apache 2.0), klein-9b (9B), klein-9b-kv (9B, KV-cached I2I)"
  )
  ...
  @Option(
    name: .long,
    help: "Image to analyze with VLM and inject description into prompt (semantic interpretation)")
  var interpret: [String] = []

  @Option(
    name: .long, help: "Save intermediate images at each N steps (e.g., 5 saves every 5 steps)")
  var checkpoint: Int?
  ...
  @Option(
    name: .long, help: "LoRA config JSON file (alternative to --lora, includes scheduler overrides)"
  )
  ...
  func run() async throws {
    ...
    let isGatedModel =
      modelVariant == .dev || modelVariant == .klein9B || modelVariant == .klein9BKV
    ...
    if token == nil && isGatedModel {
      ...
      print(
        "   1. Accept the license at https://huggingface.co/black-forest-labs/\(modelVariant == .dev ? "FLUX.2-dev" : "FLUX.2-klein-9B")"
      )
    ...
    guard let textQuantization = MistralQuantization(rawValue: textQuant) else {
      throw ValidationError(
        "Invalid text quantization: \(textQuant). Use bf16, 8bit, 6bit, or 4bit")
    }

    guard let transformerQuantization = TransformerQuantization(rawValue: transformerQuant) else {
      throw ValidationError(
        "Invalid transformer quantization: \(transformerQuant). Use bf16, qint8, or int4")
    }
    ...
    default:
      throw ValidationError(
        "Invalid memory profile: \(memoryProfile). Use auto, conservative, balanced, or performance"
      )
    }
    ...
    if let loraConfig = loraConfig {
      do {
        ...
        print(
          "LoRA loaded: \(info.numLayers) layers, rank \(info.rank), \(String(format: "%.1f", info.memorySizeMB)) MB"
        )
        if info.targetModel != .unknown {
          if info.targetModel.rawValue != modelVariant.rawValue {
            print(
              "⚠️  Warning: LoRA was trained for \(info.targetModel.rawValue), but using \(modelVariant.rawValue)"
            )
          }
        }
      } catch {
...
struct ImageToImage: AsyncParsableCommand {
  ...
  @Option(
    name: .long,
    help:
      "Image to analyze with VLM and inject description into prompt (not used as visual reference)")
  var interpret: [String] = []
  ...
  @Option(
    name: .shortAndLong,
    help: "Number of effective denoising steps (default: 28 for Dev, 4 for Klein)")
  var steps: Int?
  ...
  func run() async throws {
    ...
    guard !images.isEmpty && images.count <= maxImages else {
      if images.isEmpty {
        ...
        throw ValidationError(
          "Maximum \(maxImages) reference images allowed for \(modelVariant.displayName)")
      }
    }
...
struct Download: AsyncParsableCommand {
  ...
  func run() async throws {
    ...
    if all {
      ...
      let variants = ModelRegistry.TransformerVariant.allCases.filter {
        $0.modelType == modelVariant
      }
  ...
  private func downloadComponent(
    _ downloader: Flux2ModelDownloader, _ component: ModelRegistry.ModelComponent
  ) async throws {
    do {
...
func saveImage(_ image: CGImage, to path: String) throws {
  let url = URL(fileURLWithPath: path)
  let utType: CFString =
    path.hasSuffix(".png") ? UTType.png.identifier as CFString : UTType.jpeg.identifier as CFString

  ...
  struct ImageToImageView: View {
    ...
    private var referenceImagesSection: some View {
      VStack(alignment: .leading, spacing: 12) {
        ...
        Text(
          "Drop images here or click to add. The model will use these as reference for generation."
        )
        .font(.caption)
    ...
    private var interpretImagesSection: some View {
      VStack(alignment: .leading, spacing: 8) {
        ...
        Text(
          "VLM will analyze these images and inject descriptions into the prompt. Different from reference images - these provide semantic context only."
        )
        .font(.caption2)
        .foregroundStyle(.secondary)
    ...
    private var canGenerate: Bool {
      !viewModel.prompt.isEmpty && !viewModel.referenceImages.isEmpty && !viewModel.isGenerating
    ...
    private func selectImage() {
      ...
      if panel.runModal() == .OK {
        for url in panel.urls.prefix(
          viewModel.selectedModel.maxReferenceImages - viewModel.referenceImages.count)
        {

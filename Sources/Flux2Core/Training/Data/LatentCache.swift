...
public final class LatentCache: @unchecked Sendable {
  ...
  private func cacheFilePath(for filename: String, width: Int, height: Int) -> URL {
    ...
    return cacheDirectory.appendingPathComponent(
      "\(baseName)_\(width)x\(height)_latent.safetensors")
  }
  ...
  public func preEncodeDataset(
    ...
    for (index, sample) in dataset.enumerated() {
      ...
      if FileManager.default.fileExists(atPath: cacheFile.path)
        || cachedFilenames.contains(baseName)
      {
        cachedCount += 1
    ...
    Flux2Debug.log(
      "[LatentCache] Pre-encoded \(totalEncoded) latents (batched \(uncachedCount) new)")
  ...
  public func preEncodeControlImages(
    ...
    guard
      let controlFiles = try? fm.contentsOfDirectory(
        at: controlPath, includingPropertiesForKeys: nil)
    else {
    ...
    Flux2Debug.log(
      "[LatentCache] Found \(controlImagesByName.count) control images in \(controlPath.path)")
    ...
    for sample in targetDataset.sampleMetadata {
      ...
      // Check cache first
      let cacheFile = controlCacheDir.appendingPathComponent(
        "\(baseName)_\(targetDims.width)x\(targetDims.height)_control.safetensors")
      ...
      guard let imageSource = CGImageSourceCreateWithURL(controlImageURL as CFURL, nil),
        ...
        Flux2Debug.log(
          "[LatentCache] Failed to load control image: \(controlImageURL.lastPathComponent)")
        continue
      }
      ...
      let imageArray = imageToMLXArray(
        resizedImage, width: targetDims.width, height: targetDims.height)
    ...
    Flux2Debug.log(
      "[LatentCache] Pre-encoded \(controlLatents.count) control latents (\(alreadyCached) from cache, \(matched - alreadyCached) new)"
    )

    if matched < targetDataset.count {
      Flux2Debug.log(
        "[LatentCache] Warning: Only \(matched)/\(targetDataset.count) targets have matching control images"
      )
    }

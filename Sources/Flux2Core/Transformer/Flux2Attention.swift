...
public class Flux2Attention: Module, @unchecked Sendable {
  ...
  public func callAsFunction(
    ...
    Flux2Debug.verbose(
      "Flux2Attention input - img: \(hiddenStates.shape), txt: \(encoderHiddenStates.shape)")
    ...
    Flux2Debug.verbose(
      "Text Q/K/V projected: addedQ=\(addedQ.shape), addedK=\(addedK.shape), addedV=\(addedV.shape)"
    )
    ...
    Flux2Debug.verbose(
      "Text after reshape: addedQ=\(addedQ.shape), addedK=\(addedK.shape), addedV=\(addedVReshaped.shape)"
    )
    ...
    Flux2Debug.verbose(
      "Concatenated for attention: Q=\(concatQ.shape), K=\(concatK.shape), V=\(concatV.shape)")
    ...
    Flux2Debug.verbose(
      "Final outputs: img=\(hiddenStatesOut.shape), txt=\(encoderHiddenStatesOut.shape)")
  ...
  public func callWithKVExtraction(
    ...
    return (
      hiddenStates: hiddenStatesOut, encoderHiddenStates: encoderHiddenStatesOut, cache: cacheEntry
    )
  }

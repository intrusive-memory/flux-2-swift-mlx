...
public final class FluxTextEncoders: @unchecked Sendable {
  ...
  public static let version = "3.0.2-dev"
  ...
  public func loadVLMModel(from path: String) throws {
    ...
    if debug {
      print("[Core] Loading VLM from \(path)")
      fflush(stdout)
    }
    ...
    if debug {
      print("[Core] VLM loading complete!")
      fflush(stdout)
    }
  }
  ...
  public func loadKleinModel(
    ...
    guard let modelInfo = TextEncoderModelRegistry.shared.qwen3Model(withVariant: modelVariant)
    else {
  ...
  public func chat(
    ...
    return try generator.chat(
      messages: messages, parameters: parameters, stream: stream, onToken: onToken)
  }
  ...
  private func logInferenceMemory(_ label: String) {
    guard FluxProfiler.shared.isEnabled || ProcessInfo.processInfo.environment["VLM_DEBUG"] != nil
    else { return }
    ...
    print(
      "[VLM-INF] \(label): MLX=\(String(format: "%.1f", mlxMB))MB, Process=\(String(format: "%.1f", procMB))MB"
    )
    fflush(stdout)
  }
    ...
    public func analyzeImage(
      ...
      return try analyzeImage(
        image: cgImage, prompt: prompt, systemPrompt: systemPrompt, parameters: parameters,
        onToken: onToken)
    }
  #endif
  ...
  public func analyzeImage(
    ...
    if debug {
      print("[Analyze] Image encoded: \(patchesH)x\(patchesW) patches")
      fflush(stdout)
    }
    ...
    if debug {
      ...
      print(
        "[Debug] Last position logits: mean=\(logitsMean), std=\(logitsStd), min=\(logitsMin), max=\(logitsMax)"
      )
    ...
    for i in 0..<maxTokens {
      ...
      let nextToken = sampleToken(
        logits: nextTokenLogits, parameters: parameters, generatedTokens: generatedTokens)
  ...
  public func extractFluxEmbeddingsWithImage(
    ...
    if debug {
      print("[FLUX I2I] Starting with prompt: \(prompt)")
      fflush(stdout)
    }
    ...
    if debug {
      print("[FLUX I2I] Image will generate \(numImageTokens) tokens")
      fflush(stdout)
    }
    ...
    if debug {
      print(
        "[FLUX I2I] Total sequence length: \(inputTokens.count) tokens (image: \(numImageTokens))")
      fflush(stdout)
    }
    ...
    if debug {
      print("[FLUX I2I] Embeddings shape: \(embeddings.shape)")
      fflush(stdout)
    }

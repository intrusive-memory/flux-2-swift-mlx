...
@Suite(.serialized) struct ModelDirectoryTests {
  ...
  @Test func findModelPathChecksCustomCacheDirectory() throws {
    ...
    try "{}".write(
      to: hubCacheDir.appendingPathComponent("model_index.json"), atomically: true, encoding: .utf8)
    ...
    #expect(
      vaePath.path.hasPrefix(tempDir.path), "VAE path should use custom directory: \(vaePath.path)")
  }
